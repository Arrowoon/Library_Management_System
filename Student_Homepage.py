...
class StudentHome:
    def __init__(self,root):
        ...
        btn_book=Button(M_Frame,text="Book Details",font=("times new roman",15,"bold"),bg="#0b5377",fg="white", cursor="hand2", command=self.Book_Details).place(x=20,y=5,width=150,height=40)

        btn_issued = Button(M_Frame, text="Issue Report", font=("times new roman", 15, "bold"), bg="#0b5377", fg="white",
                          cursor="hand2",command=self.Issue_Report).place(x=220, y=5, width=150, height=40)
        btn_returned = Button(M_Frame, text="Return Report", font=("times new roman", 15, "bold"), bg="#0b5377", fg="white",
                          cursor="hand2",command=self.Return_Report).place(x=425, y=5, width=150, height=40)
        btn_search = Button(M_Frame, text="Google Search", font=("times new roman", 15, "bold"), bg="#0b5377",
                              fg="white",
                              cursor="hand2", command=self.Google_Search).place(x=630, y=5, width=150, height=40)
        btn_logout = Button(M_Frame, text="Logout", font=("times new roman", 15, "bold"), bg="#0b5377", fg="white",
                          cursor="hand2",command=self.logout).place(x=840, y=5, width=150, height=40)
        btn_exit = Button(M_Frame, text="Exit", font=("times new roman", 15, "bold"), bg="#0b5377", fg="white",
                          cursor="hand2",command=self.exit_).place(x=1050, y=5, width=150, height=40)
    ...
    def Google_Search(self):
        self.new_win = Toplevel(self.root)
        self.new_obj = Google(self.new_win)
